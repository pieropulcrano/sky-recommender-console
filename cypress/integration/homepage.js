...
describe('Testing Home Page', () => {
    ...
    it('Check navigation Tabs', () => {
        ...
        cy.get('[data-test="nav-tabs"]').within(() => {
            ...
            cy.get('button').first().should('have.attr', 'aria-selected').and('equal', 'true');
            //il secondo è fallback e non dovrebbe essere selezionato 
            cy.get('button').last().should('have.text', 'Fallback');
            cy.get('button').last().should('have.attr', 'aria-selected').and('equal', 'false');
            ...
            cy.get('button').first().should('have.text', 'Schedule');
            cy.get('button').first().should('have.attr', 'aria-selected').and('equal', 'false');
            //il secondo è fallback e dovrebbe essere selezionato 
            cy.get('button').last().should('have.text', 'Fallback');
            cy.get('button').last().should('have.attr', 'aria-selected').and('equal', 'true');
