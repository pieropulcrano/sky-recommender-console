...
describe('Testing vod raccomandation', () => {
    ...
    it('Check creation VOD empity form', () => {
        ...
        cy.get('.empity-slot').each(($el, index, $list) => {
            ...
            cy.wrap($el).find('[data-testid="AddCircleIcon"] > path').click({ force: true });
    ...
    it('Check creation VOD using prev data', () => {
        ...
        let dateToSearch = cy.generateFutureDate(1, 'DD/MM/YYYY h:mm A')
        let dateToSend = encodeURIComponent(Cypress.dayjs(dateToSearch,'DD/MM/YYYY h:mm A').format('ddd MMM DD YYYY'));//todo da risistemare formato in accordo con server
        let randomIndexCluster = cy.getRandomNumber(1, 2);
        let randomIndexClusterVal = "C" + randomIndexCluster;
        let interceptedCreate = false;
        //intercept get search rec vod
        cy.intercept({ method: 'GET', path: '/recommendations*' }).as('searchRequest');
        //intercept update search rec vod
        cy.intercept({ method: 'POST', path: '/recommendations' }, (req) => {
            req.continue((res) => {
                interceptedCreate = true;
        ...
        cy.get('@searchRequest').its('request.url').should('to.match', new RegExp('cluster=' + randomIndexClusterVal));
        //check if contains correct parameter validFrom_lte
        cy.get('@searchRequest').its('request.url').should('to.match', new RegExp('validFrom_lte=' + dateToSend));
        //check type on request
        cy.get('@searchRequest').its('request.url').should('to.match', new RegExp('type=VOD'));
        ...
        cy.wait('@searchRequest').should(({ req, response }) => {
            ...
            cy.get('.prev-vod-slot').should('have.length', body.recommendation.length);
