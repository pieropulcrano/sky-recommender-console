...
describe('Testing Fallback Page', () => {
    ...
    beforeEach(() => {
        ...
        cy.visit('http://localhost:3000/fallback');
    ...
    it('Check All fallbacks', () => {
        ...
        cy.request('GET', 'http://localhost:3001/fallback-vod-recommendation')
            .then((response) => {
                ...
                for (var i = 0; i < fallbacks.length; i++) {
                    ...
                    cy.get('@event').find('[data-test="event-title"]').should('have.text', fallbacks[i].title);
                    cy.get('@event').find('[data-test="event-startProgram"]').should('not.be.empty');
                    cy.get('@event').find('[data-test="event-endProgram"]').should('not.be.empty');
    ...
    it('Check fallback events', () => {
        ...
        cy.get('[data-testid="AddCircleIcon"] > path').should('have.length', 1).click({ force: true });
    ...
    it('Check clear button', () => {
        //check exist
        cy.get('.MuiButton-containedPrimary').as('clearButton').should('have.length', 1).click({ force: true });
